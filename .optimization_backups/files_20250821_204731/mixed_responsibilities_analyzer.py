...
# TODO: Consider extracting this block into a separate method # Tracked: 2025-08-21
    ...
    def analyze_all_files(self) -> Dict[str, Any]:
        ...
        logging.info("🎯 ANALYZING MIXED RESPONSIBILITIES...")
        logging.info("=" * 60)
...
# TODO: Consider extracting this block into a separate method # Tracked: 2025-08-21
    ...
    def print_analysis_summary(self):
        ...
        logging.info(f"   Files analyzed: {report['summary']['files_analyzed']}")
        logging.info(f"   Functions analyzed: {report['summary']['functions_analyzed']}")
        logging.info(f"   SRP violations found: {report['summary']['violations_found']}")
        logging.info(f"   Critical: {report['summary']['critical_violations']}")
        logging.info(f"   High: {report['summary']['high_violations']}")
        logging.info(f"   Medium: {report['summary']['medium_violations']}")
        
        logging.info(f"\n🔍 TOP 12 VIOLATIONS TO FIX:")
        for i, violation in enumerate(report['top_violations'][:12], 1):
            logging.info(f"   {i}. {violation.function_name} ({violation.file_path})")
            logging.info(f"      Responsibilities: {', '.join(violation.responsibilities)}")
            logging.info(f"      Severity: {violation.severity}")
            logging.info(f"      Refactor: {violation.suggested_refactor}")
            logging.info()
        
        logging.info(f"🎯 RESPONSIBILITY FREQUENCY:")
        for resp, count in sorted(report['responsibility_frequency'].items(), key=lambda x: x[1], reverse=True):
            logging.info(f"   {resp}: {count} functions")
...
def main():
    ...
    logging.info("🎯 MIXED RESPONSIBILITIES ANALYZER - PHASE 10")
    logging.info("=" * 60)
    logging.info("Identifying 12 functions that violate Single Responsibility Principle\n")
    ...
    logging.info(f"📈 NEXT: Create systematic refactoring plan for top 12 violations")
