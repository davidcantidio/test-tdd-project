# tests/migrations/test_2025_09_02_epic_dependencies_RED.py
"""
A1. [DB] GREEN — Tabela de dependências de épicos

Após aplicar a migration `m_2025_09_02_create_epic_dependencies.py`, validamos:
  - Tabela existe e aceita inserção válida
  - Auto-dependência falha (IntegrityError)
  - Cross-project falha (IntegrityError)
  - Duplicidade (UNIQUE) falha (IntegrityError)
  - FKs inválidas falham (IntegrityError)
"""
...
def test_dependencies_constraints(tmp_path: Path):
    ...
    try:
        ...
        cur = conn.cursor()
        cur.execute(
            "INSERT INTO framework_epic_dependencies (project_id, epic_id, depends_on_epic_id, dep_type) "
            "VALUES (1, 1, 2, 'blocks')"
        )
        conn.commit()
        ...
        with pytest.raises(sqlite3.IntegrityError):
            conn.execute(
                "INSERT INTO framework_epic_dependencies (project_id, epic_id, depends_on_epic_id) "
                "VALUES (1, 1, 1)"
            )
            conn.commit()

        # 4) Cross-project mismatch must fail
        # epic 1 (project 1) depende de epic 3 (project 2) mas project_id=1 => mismatch
        with pytest.raises(sqlite3.IntegrityError):
            conn.execute(
                "INSERT INTO framework_epic_dependencies (project_id, epic_id, depends_on_epic_id) "
                "VALUES (1, 1, 3)"
        ...
        # 5) FK inválida: epic_id inexistente
        with pytest.raises(sqlite3.IntegrityError):
            conn.execute(
                "INSERT INTO framework_epic_dependencies (project_id, epic_id, depends_on_epic_id) "
                "VALUES (1, 999, 2)"
        ...
        # 6) FK inválida: depends_on_epic_id inexistente
        with pytest.raises(sqlite3.IntegrityError):
            conn.execute(
                "INSERT INTO framework_epic_dependencies (project_id, epic_id, depends_on_epic_id) "
                "VALUES (1, 1, 999)"
        ...
        # Sanidade: tabela ficou só com a dependência válida
        rows = conn.execute(
            "SELECT project_id, epic_id, depends_on_epic_id, dep_type "
            "FROM framework_epic_dependencies"
        ).fetchall()
