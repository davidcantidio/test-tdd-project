...
class GoogleOAuthManager:
    ...
    def _stash_with_ttl(self, key: str, value: str, ttl_seconds: int) -> None:
        ...
        print(f"🔐 OAUTH DEBUG: Storing {key} with TTL {ttl_seconds}s - value: {value}")
        self.store.set(key, payload)
        # Verify storage immediately
        stored = self.store.get(key)
        print(f"🔐 OAUTH DEBUG: Verified storage for {key}: {bool(stored)}")

    def _pop_if_valid(self, key: str, candidate: str) -> bool:
        print(f"🔐 OAUTH DEBUG: Checking {key} - candidate: {candidate}")
        payload = self.store.get(key)
        print(f"🔐 OAUTH DEBUG: Retrieved payload for {key}: {payload}")
        
        if not payload:
            print(f"🔐 OAUTH DEBUG: No payload found for {key}")
            return False
            
        try:
            exp = float(payload["exp"])
            val = str(payload["value"])
            print(f"🔐 OAUTH DEBUG: {key} - stored value: {val}, expires: {exp}")
        except Exception as e:
            print(f"🔐 OAUTH DEBUG: Payload parsing failed for {key}: {e}")
            self.store.delete(key)
            return False
            
        current_time = _now().timestamp()
        print(f"🔐 OAUTH DEBUG: {key} - current time: {current_time}, expires: {exp}")
        
        if current_time > exp:
            print(f"🔐 OAUTH DEBUG: {key} expired - deleting")
            self.store.delete(key)
            return False
            
        ok = hmac.compare_digest(val, candidate)
        print(f"🔐 OAUTH DEBUG: {key} validation result: {ok}")
        self.store.delete(key)
        return ok
    ...
    def handle_callback(self, authorization_code: str, state: str) -> Dict[str, Any]:
        ...
        print(f"🔐 OAUTH DEBUG: Starting callback - flow_id: {flow_id}")
        print(f"🔐 OAUTH DEBUG: Session keys before callback: {list(st.session_state.keys()) if hasattr(st, 'session_state') else 'NO_SESSION'}")
        print(f"🔐 OAUTH DEBUG: Received code: {authorization_code[:20]}...")
        ...
        if not self._pop_if_valid("oauth_state", state):
            ...
            for k in ("oauth_code_verifier", "oauth_nonce"):
                self.store.delete(k)
            print(f"🔐 OAUTH ERROR: Invalid OAuth state - CSRF protection triggered")
            raise ValueError("Estado OAuth inválido/expirado (CSRF)")

        print(f"🔐 OAUTH DEBUG: State validation passed, getting code verifier...")
        ...
        print(f"🔐 OAUTH DEBUG: Code verifier retrieved: {bool(code_verifier)}")
        self.store.delete("oauth_code_verifier")

        print(f"🔐 OAUTH DEBUG: Creating OAuth flow for token exchange...")
        try:
            flow = self._create_flow(code_verifier=code_verifier)
            print(f"🔐 OAUTH DEBUG: Flow created successfully, fetching token...")
            
            flow.fetch_token(code=authorization_code)
            cred: Credentials = flow.credentials
            print(f"🔐 OAUTH DEBUG: Token exchange SUCCESS - got credentials")
            
        except Exception as e:
            print(f"🔐 OAUTH ERROR: Token exchange FAILED: {str(e)}")
            print(f"🔐 OAUTH ERROR: Exception type: {type(e).__name__}")
            import traceback
            print(f"🔐 OAUTH ERROR: Full traceback:")
            traceback.print_exc()
        ...
        print(f"🔐 OAUTH DEBUG: Resolving user info from credentials...")
        try:
            user_info, access_expiry_iso = self._resolve_user_info_and_expiry(cred)
            print(f"🔐 OAUTH DEBUG: User info resolved successfully: {user_info.get('name', 'NO_NAME')}")
        except Exception as e:
            print(f"🔐 OAUTH ERROR: User info resolution FAILED: {str(e)}")
            import traceback
            print(f"🔐 OAUTH ERROR: User info traceback:")
            traceback.print_exc()
            raise RuntimeError(f"Falha ao resolver informações do usuário: {e}") from e

        # Mapeia sessão (sem client_secret/token_uri)
        print(f"🔐 OAUTH DEBUG: Creating session data...")
        try:
            ...
            print(f"🔐 OAUTH DEBUG: Session data created, saving to store...")
            
            self.store.set("authenticated", True)
            print(f"🔐 OAUTH DEBUG: Authenticated flag set to True")
            
            self.store.set("user_session", session_data)
            print(f"🔐 OAUTH DEBUG: User session data saved to store")
            
            _json_log(self.logger, "login_success", flow_id=flow_id, email=user_info.get("email"), sub=user_info.get("id"))
            print(f"🔐 OAUTH DEBUG: Login success logged")
            
        except Exception as e:
            print(f"🔐 OAUTH ERROR: Session data creation/saving FAILED: {str(e)}")
            import traceback
            print(f"🔐 OAUTH ERROR: Session save traceback:")
            traceback.print_exc()
            raise RuntimeError(f"Falha ao salvar dados da sessão: {e}") from e

        print(f"🔐 OAUTH DEBUG: Session data saved successfully")
        print(f"🔐 OAUTH DEBUG: Session keys after callback: {list(st.session_state.keys()) if hasattr(st, 'session_state') else 'NO_SESSION'}")
        print(f"🔐 OAUTH DEBUG: Authenticated flag: {self.store.get('authenticated')}")
        print(f"🔐 OAUTH DEBUG: User session exists: {bool(self.store.get('user_session'))}")
        print(f"🔐 OAUTH DEBUG: User name: {user_info.get('name', 'NO_NAME')}")
    ...
    def is_authenticated(self) -> bool:
        auth_flag = self.store.get("authenticated")
        sess = self.store.get("user_session")
        
        print(f"🔐 OAUTH DEBUG: is_authenticated check")
        print(f"🔐 OAUTH DEBUG: - authenticated flag: {auth_flag}")
        print(f"🔐 OAUTH DEBUG: - user_session exists: {bool(sess)}")
        print(f"🔐 OAUTH DEBUG: - session keys: {list(st.session_state.keys()) if hasattr(st, 'session_state') else 'NO_SESSION'}")
        
        if not auth_flag:
            print(f"🔐 OAUTH DEBUG: - RESULT: False (no auth flag)")
            return False
            
        if not isinstance(sess, dict):
            print(f"🔐 OAUTH DEBUG: - RESULT: False (invalid session)")
            return False
            
        try:
            exp = datetime.fromisoformat(sess["expires_at"])
            if _now() > exp:
                print(f"🔐 OAUTH DEBUG: - RESULT: False (session expired)")
                self.logout()
                return False
        except Exception as e:
            print(f"🔐 OAUTH DEBUG: - RESULT: False (session error: {e})")
            self.logout()
            return False
            
        print(f"🔐 OAUTH DEBUG: - RESULT: True (authenticated)")
        return True
    ...
    def render_login_page(self) -> None:
        ...
        if "code" in qp and "state" in qp:
            print(f"🔐 OAUTH DEBUG: Login page detected OAuth callback parameters")
            with st.spinner("Autenticando..."):
                try:
                    print(f"🔐 OAUTH DEBUG: Starting callback processing in render_login_page...")
                    data = self.handle_callback(qp["code"], qp["state"])
                    print(f"🔐 OAUTH DEBUG: Callback processing completed successfully!")
                    
                    # Clear OAuth parameters and show success
                    print(f"🔐 OAUTH DEBUG: Clearing OAuth query parameters...")
                    st.query_params.clear()
                    
                    st.success(f"✅ Bem-vindo(a), {data['user_info']['name']}!")
                    print(f"🔐 OAUTH DEBUG: Success message shown, triggering rerun...")
                    
                    # Directly rerun to preserve session state
                    # No redirect needed - let Streamlit handle the refresh
                    st.rerun()
                except Exception as e:
                    print(f"🔐 OAUTH ERROR: Login page callback processing FAILED: {str(e)}")
                    print(f"🔐 OAUTH ERROR: Exception type: {type(e).__name__}")
                    import traceback
                    print(f"🔐 OAUTH ERROR: Login page traceback:")
                    traceback.print_exc()
...
def is_user_authenticated() -> bool:
    print(f"🔐 OAUTH DEBUG: is_user_authenticated() called")
    
    if not DEPS:
        print(f"🔐 OAUTH DEBUG: - DEPS not available, using traditional auth")
        return _is_traditional_auth_active()
        
    auth = GoogleOAuthManager()
    print(f"🔐 OAUTH DEBUG: - OAuth manager configured: {auth.configured}")
    print(f"🔐 OAUTH DEBUG: - OAuth manager client_id: {auth.cfg.client_id[:20]}..." if auth.cfg.client_id else "🔐 OAUTH DEBUG: - OAuth manager client_id: EMPTY")
    
    # Always check OAuth session first, even if not configured
    # This ensures OAuth sessions persist correctly after reruns
    is_auth = auth.is_authenticated()
    if is_auth:
        print(f"🔐 OAUTH DEBUG: - FINAL RESULT: True (OAuth authenticated)")
        return True
        
    # Only use fallback if no OAuth session exists
    if not auth.configured and auth.cfg.allow_dev_fallback:
        print(f"🔐 OAUTH DEBUG: - Using fallback traditional auth")
        result = _is_traditional_auth_active()
        print(f"🔐 OAUTH DEBUG: - FINAL RESULT: {result} (traditional auth)")
        return result
        
    print(f"🔐 OAUTH DEBUG: - FINAL RESULT: False (no authentication)")
    return False
